...
fn main() -> std::io::Result<()>{
    ...
    let filename = "/home/justintilman/Programmes_perso/toudou/toudou.txt";
    
    let mut tasklist = file_read_write::read_lines(filename);
    
    // If no arguments: print the tasks to the screen
    if cli_args.add_flag.is_none() && cli_args.remove_flag.is_none() {
        let mut index: i8 = 1;
    ...
    // If requested to delete a task
    if cli_args.remove_flag.is_some() {
        tasklist.remove(cli_args.remove_flag.unwrap() - 1);
        file_read_write::write_tasklist_to_file(&tasklist, filename)?;
    }
